...
void BuildSubGraph(const char * p_s_input_file, const std::vector<size_t> & vertex_subset,
    ...
    size_t nidx = 0;
    // old id -> new id
    std::map<size_t, size_t> map_cam_subset, map_lmk_subset, map_lmk_tmp;
            ...
            if (words.size() > 0) {
                if (words[0] == "VERTEX_CAM") {
                    ...
                    if (map_cam_subset.find(kf_id) == map_cam_subset.end())
                        continue ;
                    if (map_lmk_tmp.find(lmk_id) != map_lmk_tmp.end())
                        continue ;
                    //                    std::cout << "insert lmk pair " << lmk_id << ", " << nidx << std::endl;
    ...
    for (const auto & pair : map_lmk_tmp)
    {
        map_lmk_subset.insert({pair.first, nidx});
        //        pair.second = nidx;
        nidx ++;
    }

    std::cout << "Done with 1st scan with total idx num " << nidx << "; start 2nd scan!" << std::endl;
        ...
        while (std::getline(fin, line)) {
            ...
            if (words.size() > 0) {
                if (words[0] == "VERTEX_CAM") {
                    ...
                    //
                    //                  std::cout << "create lmk with id " << iter->second << std::endl;
