...
int main(int argc, char **argv) {
    ...
    if(argc != 7)
    {
        cerr << endl << "Usage: ./stereo_general path_to_vocabulary path_to_settings budget_per_frame "
             << " do_viz path_to_sequence path_to_traj" << endl;
    ...
    LoadImages(string(argv[5]), vstrImageLeft, vstrImageRight, vTimeStamp);
    ...
    for(int ni=0; ni<nImages; ni++)
    {
        ...
        // double time_rest = 50*3*1e-3 - ttrack;
        // if(time_rest > 0)
    ...
    //
   // SLAM.SaveLmkLog( std::string(argv[6]) + "_Log_lmk.txt" );
...
//            cout << filename.substr(0, filename.length()-4) << endl;
    ...
    for (std::list<std::string>::iterator it_l = imgs_l.begin(); it_l != imgs_l.end(); ++ it_l) {
        while (it_r != imgs_r.end()) {
            ...
            if (fabs((double)((long double)time_stamp_l * 1e-9) - (double)((long double)time_stamp_r * 1e-9)) < 0.003) {
...
//                cout << vTimeStamps[vTimeStamps.size() - 1] << endl;
            ...
            else if ((double)((long double)time_stamp_l * 1e-9) < (double)((long double)time_stamp_r * 1e-9)) {
