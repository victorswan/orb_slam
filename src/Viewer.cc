...
void Viewer::Run()
{
    ...
    {
        unique_lock<mutex> lock(mpSystem->mMutexMode);
        bLocalizationMode = mpSystem->mbDeactivateLocalizationMode;
    }

    pangolin::CreatePanel("menu").SetBounds(0.0, 1.0, 0.0, pangolin::Attach::Pix(175));
    pangolin::Var<bool> menuFollowCamera("menu.Follow Camera", bFollow, true);
    ...
    while (1)
    {
        ...
        if (menuSaveMap)
        {
            if(mpSystem->save_map)
            {
                cout << "Save Map" << endl;
                mpSystem->archiveMap(mpSystem->map_path, mpSystem->map_filename);
            }
            menuSaveMap = false;
        }
